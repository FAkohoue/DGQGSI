...
construct_Wd_from_dg <- function(
  ...
  for (tr in trait_cols) dt[, (tr) := as.numeric(get(tr))]
  ...
  } else if (anyNA(as.matrix(dt[, ..trait_cols]))) {
  ...
  X <- as.matrix(dt[, ..trait_cols])
  if (isTRUE(center_traits)) X <- sweep(X, 2, colMeans(X), FUN = "-")
  ...
  if (is.null(base_diag)) {
    ...
    if (anyNA(base_diag)) stop("base_diag missing for traits: ", paste(trait_cols[is.na(base_diag)], collapse = ", "), call. = FALSE)
  ...
  if (method == "hybrid") {
    W_d <- lambda_diag * diag_component + lambda_outer * outer_component + lambda_corr * corr_component
  }
  ...
  W_d <- 0.5 * (W_d + t(W_d))
  if (isTRUE(positive_diagonal_only)) diag(W_d) <- pmax(diag(W_d), 0)
